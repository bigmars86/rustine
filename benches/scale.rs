...
pub fn generate_grammar() -> String {
    ...
    for sect in 0..NUM_SECTIONS {
        let _ = write!(s, "    match 'sect{sect:02}' nl:\n        out.enter('sect{sect:02}')\n        s{sect:02}_dispatch()\n");
    }
    s.push('\n');
    s
}
...
fn print_peak_rss(label: &str) {
    #[cfg(target_os = "windows")]
    {
        ...
        #[repr(C)]
        struct ProcessMemoryCounters { cb: u32, page_fault_count: u32, peak_working_set_size: usize, working_set_size: usize, _rest: [usize; 6] }
        extern "system" {
...
fn bench_scale(c: &mut Criterion) {
    ...
    eprintln!("[scale] Input: {:.1} MB, Grammar: {} bytes", input.len() as f64 / 1_048_576.0, grammar_src.len());
    ...
    for (fmt_name, fmt) in &formats {
        ...
        group.bench_function(format!("serialize_{fmt_name}_25MB"), |b| {
            b.iter(|| { let out = serialize_tree(&result, *fmt); criterion::black_box(out.len()); });
        });
        group.finish();
    }
